...
function chatShow() {
  document.getElementById('chat').innerHTML = "<div id='chatFenetre'><div id='nameDiv'></div><div id='conversation'></div><div id='inputArea'><input type='text' placeholder='message' id='message' onkeydown='chatListener(event)' autofocus><button class='messageSubmit' onclick='send()' id='chatBtn'>&#10148;</button></div></div>";
...
function chatContentLoader(){
  for (var i = 1; i < parseInt(localStorage.messageId) + 1; i++) {
    ...
    }else if(localStorage.getItem(i + "author").length == 0){
      localStorage.messageId = i - 1;
    }else{
    ...
    if(localStorage.getItem(i + "cat") == 'q'){
      ...
      if(localStorage.getItem(i + 'code') == 'photo'){
        document.getElementById(i + 'questionCorpus').innerHTML += "<input type='file' name='img' id='imgInput" + i + "' onclick='changedPictures(" + i + ")'><button onclick='useCamera(" + i + ")'>Use your Camera</button><button id='uniqueBtn' onclick='sendPictures(" + i + ")' class='btn btn-success'>&#9658;</button><canvas style='display:none' id='canvas" + i + "'></canvas><img class='apercu' id='output" + i + "'>";
      }else{
...
function displayModal(messageId, imgId){
  ...
  document.getElementById('modalContent').innerHTML = "<div class='west'><img class='modalImage' id='img01'><div id='coment' onclick='toggleComentExtention()'><h2>DESCRIPTION</h2></div><div id='comentIndicator' onclick='toggleComentExtention()'>&#9663;</div></div> <div id='caption' class='east'>...</div>";
...
function send() {
  ...
  if(array.indexOf(document.getElementById('message').value) < 0){
...
function useCamera(messageId){
  ...
  document.getElementById('modalContent').innerHTML = "<pre id='preLog'>Chargement…</pre><video id='video' autoplay='autoplay'></video><input type='button' id='buttonSnap' value='take a picture' disabled='disabled' onclick='snapshot(" + messageId + ")' />";
...
function snapshot(messageId)
{
  var modal = document.getElementById('presentation');
  var canvas = document.getElementById('canvas' + messageId);

  canvas.width = video.videoWidth;
  canvas.height = video.videoHeight;
  canvas.getContext('2d').drawImage(video, 0, 0);

  userClick({message: 'takenPicture'});

  stop();
  modal.style.display = "none";

  sendWebcam(messageId);
}
...
function sendWebcam(messageId){
  var input, canvas, context, output;
  input = document.getElementById("imgInput" + messageId);
  canvas = document.getElementById("canvas" + messageId);
  context = canvas.getContext('2d');
  output = document.getElementById("output" + messageId);
